...
const suggestList: HTMLUListElement = document.getElementById("suggestions") as HTMLUListElement;
...
function updateSuggestions(matches: string[]): void {
  ...
  matches.forEach((match: string) => {
    suggestList.innerHTML += `<li tabindex="0"> ${ match } </li>`
  })
}
...
document.addEventListener("keyup", async (Event: KeyboardEvent) => {
  if (Event.key == "b") {
    console.log("Hello")
    await updateValues("Pisces", "Pisces", "Sagittarius");
    postAndUpdate()
  }
});
